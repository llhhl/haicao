...
class Diyform extends Common
{
    ...
    public function editdiyform()
    {
        ...
        if(request()->isAjax()){
            $param = input('post.');
			$param['smswarn'] = array_key_exists("smswarn", $param) ? 1 : 0;
			$param['mpwarn'] = array_key_exists("mpwarn", $param) ? 1 : 0;
...
/*    public function smsremind()
    {
        $coffile = CONF_PATH.DS.'extra'.DS.'sms.php';
        if(request()->isAjax()){
            Config::load($coffile, '', 'sms');
            $conflist = Config::get('','sms');
            $param = input('post.');  
            $param = add_slashes_recursive($param);
            $param['sms_send'] = array_key_exists("sms_send", $param) ? 1 : 0;
            setConfigfile($coffile, add_slashes_recursive(array_merge($conflist, $param)));
            return json(['code' => 1, 'data' => '', 'msg' => '更新短信设置成功']);
            exit();
        }
