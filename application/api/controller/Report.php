...

{
    ...
    public function pdf_reset(){
        ...
        $result = ReportList::where('id',$id)->update(['pdf_url'=>'']);
...

    {
        ...
        if (!empty($result['pdf_url']) && !file_exists($result['pdf_url']) ) {
            ...
            $report_url = '/uploads/pdf/' . $result['create_time']. '-'.$result['job_number']. '-' .$type_str.'-'. $result['client_mobile'] . ".pdf";
...
//            return dump($content);exit();
            ...
            if($result['moban']==2){
                ...
                $spdf->create_pdf2($result,$content);// 海草2

            }else{

                $spdf->create_pdf($result,$content);// 海草1
 ...
 public function uploadimg()
  {
		...
		//	echo("<br>date:".date('Ymd')."<br>");
	// 移动到框架应用根目录/public/uploads/ 目录
 $user_info = \app\api\model\ReportUser::where('mobile',  $phone)->find();
		if($house_type==1){
                $type_str = 'MP';
            }else{
                $type_str = 'JZ';
        ...
        $path = date("Y_m_d").'-'.$user_info['job_number'].'_'.$type_str.'-' .$phone;       
		
	$path = './uploads/image/'. $path;

	
	$info = $file->validate(['size' => 10240000, 'ext' => 'jpg,png,gif'])->move($path);
	
	if($info){	           
     $filename=$path."/".date('Ymd')."/".$info->getFilename();//当前图片位置及名称	 不知道为什么系统自动生成了一个日期文件夹
	// echo("<br>filename=".$filename);
	  $image = Image::open($filename);
    $image->thumb(600,450,Image::THUMB_FIXED)->save($filename);
   ...
   return($filename);
...

    {
        ...
        $path_folder = date("Y_m_d").'-'.$user_info['job_number'].'-'.date('H').'.'.date('i').'.'.date('s').'_'.$type_str.'-' . $form_data['client_mobile'];

        $path = 'uploads/image/' .$path_folder.'/';//  文件夹 uploads/image/2020_07_02-10001-JZ-1301300000 命名为 年_月_日-员工编号-房屋类型-手机号
        ...
        $form_data['image_folder']=$path_folder;
        ...
        if($result){
            ...
            $r = $this->download_interior($result->id);
            ...
            if($r){
                ...
                if($form_data['sc']==1){
                    ...
                    delDir('./uploads/image/'.$form_data['image_folder']);
    ...
    public function download_interior($id)
...
// set default header data
        ...
        // $pdf->SetHeaderData("", "","海草验房|中国第三方专业验房平台 3333                                    ", "");
...
//Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M')
        ...
        $create_time = date('Y-m-d',$data['create_time']);
